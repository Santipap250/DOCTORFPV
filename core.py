...
from logic.firmware_compat import normalize_version, resolve_tier, TIER_LABELS, cli_param_names
...
def _parse_analysis_form():
    ...
    bf_version_raw    = request.form.get("bf_version", "4.4.3")
    bf_version        = normalize_version(bf_version_raw) or "4.4.3"
...
def _handle_analysis_get_params():
    ...
    bf_version  = normalize_version(a.get("bf_version")) or "4.4.3"
    ...
    try:
        analysis["cli_meta"] = {**cli_param_names(bf_version), "cli": build_cli_diff(analysis, firmware_version=bf_version)}
    except Exception:
        logger.exception("Firmware-aware GET CLI generation error")
        analysis["cli_meta"] = {**cli_param_names(bf_version), "cli": "# CLI generation unavailable — check server logs"}
    return analysis
...
def _handle_analysis_post():
    ...
    bf_version            = p.get("bf_version") or "4.4.3"
    ...
    bf_version = normalize_version(bf_version) or "4.4.3"
    tier = resolve_tier(bf_version)
    analysis["firmware_version"] = bf_version
    analysis["firmware_tier"] = tier
    analysis["firmware_label"] = TIER_LABELS[tier]
    try:
        ...
        logger.exception("Firmware-aware POST CLI generation error")
        analysis["cli_meta"] = {**cli_param_names(bf_version), "cli": "# CLI generation unavailable — check server logs"}
